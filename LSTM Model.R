...
## Function LSTM.test.training
## Parameters:  
## list: data input
## epochs: number of epochs
## units: number of units within the cell

library(keras)
library(yardstick)
library(dplyr)

LSTM.test.training<-function(list,epochs,units){
  ...
  for(i in 1:length(list)){
    ...
    #Training
    df.training<-df%>%filter(SET.IS == "TRAINING")%>%select(c(PAY_1:DEFAULT))
    df.training.targer<-as.matrix(df.training%>%select(DEFAULT))
    df.training<-df.training%>%select(PAY_1:PAY_AMT6)
    df.training<-as.matrix(df.training)
    dimnames(df.training)<-NULL
    df.training<-df.training%>% array(dim = c(nrow(df.training),6,3)) 
    
    
    #Validation
    df.validation<-df%>%filter(SET.IS == "TEST")%>%select(c(PAY_1:DEFAULT))
    df.validation.targer<-as.matrix(df.validation%>%select(DEFAULT))
    df.validation<-df.validation%>%select(PAY_1:PAY_AMT6)
    df.validation<-as.matrix(df.validation)
    dimnames(df.validation)<-NULL
    df.validation <- df.validation %>% array(dim = c(nrow(df.validation),6,3))  #input shape
    ...
    model<-keras_model_sequential()%>%
      layer_lstm(units = units, input_shape = c(6,3))%>% #input shape
...
#The data set with target mean encoding was used, but for this model the results are 
# the same since only the dynamic variables are being used.

results.lstm.targetmean.500.5<-LSTM.test.training(kfold.targetMean.scale,500,5) 
results.lstm.targetmean.500.10<-LSTM.test.training(kfold.targetMean.scale,500,10) 
results.lstm.targetmean.1500.15<-LSTM.test.training(kfold.targetMean.scale,1500,15)
results.lstm.targetmean.2000.20<-LSTM.test.training(kfold.targetMean.scale,2000,20)
